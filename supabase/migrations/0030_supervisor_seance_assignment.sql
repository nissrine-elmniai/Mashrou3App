-- MIGRATION 0030 — lier la séance au superviseur à l'activation de l'invitation
-- À exécuter après 0029_separate_role_auth_emails.sql
...
begin
  ...
  if private.profile_has_role(new.id, 'supervisor') then
    ...
    perform public.assign_supervisor_seance_from_invitation(new.id, v_canonical);
  end if;
  return new;
end;
$$;

-- Rattrapage : superviseurs déjà activés mais sans séance assignée
do $$
declare
  r record;
begin
